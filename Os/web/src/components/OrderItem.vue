...
<!--          订单状态-->
          ...
          <template v-else-if="order.state==='已失效'">
            <td style="color: #3b62f8"> {{order.state}}</td>
            <td>
              <el-button style="font-size: 15px;" type="info" plain disabled>订单失效</el-button>
            </td>
          </template>
...
export default {
  ...
  methods:{
    //跳转到支付页面
    pay(){
      this.$router.push({path: 'pay',query:{money: this.order.total_price,orderNo: this.order.order_no,count: this.order.count,}})
    },
    //确认收货
    receive(){
