...
export default {
  ...
  methods: {
    ...
    submitOrder() {
      ...
      // 提交订单
      API.post("/api/order?goodsid=" +this.good.id, {
        ...
        if (res.code === "200") {
          ...
          var  sumprice=this.good.realPrice * this.good.num;
          sumprice = parseFloat(sumprice.toFixed(2));
          console.log(sumprice);
          //跳转到支付页面
          window.location.href = `http://localhost:9191/alipay/pay?orderId=${orderNo}&price=${sumprice}&num=${this.good.num}&goodsName=${encodeURIComponent("商品")}`;
