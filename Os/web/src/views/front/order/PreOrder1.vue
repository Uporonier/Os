...
export default {
  ...
  methods: {
    ...
    submitOrder() {
      ...
      // 提交订单
      API.post("/api/order/updateaddress?orderNo="+this.orderNo, {
        linkUser: address.linkUser,
        linkPhone: address.linkPhone,
        linkAddress: address.linkAddress,
      },{
        credentials: 'include' // 添加这一行以允许携带第三方Cookie
      }).then((res) => {
        if (res.code === "200") {
          console.log(res.data)
          //跳转到支付页面
          window.location.href = `http://localhost:9191/alipay/pay?orderId=${this.orderNo}&price=${this.sumPrice}&num=${1}&goodsName=${encodeURIComponent("商品")}`;
