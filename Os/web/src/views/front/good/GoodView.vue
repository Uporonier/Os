    ...
    methods: {
        ...
        addToCart() {
            ...
            this.request.get("/userid").then((res) => {
                ...
                this.request.post("/api/cart/addtocart", cart).then((res) => {
                    if (res.code === "200") {
      ...
      logUserEvent(goodId) {
        this.request.post('/userevents/add?goodsid='+this.goodId, ).then(response => {
          console.log('User event logged');
        }).catch(error => {
