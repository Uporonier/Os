  ...
  export default {
    ...
    methods: {
      handleSecKill(row) {
        var tmp= !!row.isSecKill;
        // 如果是秒杀商品
        if(tmp=== true)
        {
          this.setSecKill(row);//用户输入价格 库存
        }
        else {
          this.setNotSecKill(row);
        }

      },
      setSecKill(row) {
        if (confirm('确定将此商品设为秒杀商品吗？')) {
          this.$prompt('请输入秒杀价格', '设置秒杀商品价格', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            inputPattern: /^\d+(\.\d{1,2})?$/,
            inputErrorMessage: '请输入正确的价格格式'
          }).then(({ value: seckillPrice }) => {
            this.$prompt('请输入秒杀库存', '设置秒杀商品库存', {
              confirmButtonText: '确定',
              cancelButtonText: '取消',
              inputPattern: /^[1-9]\d*$/,
              inputErrorMessage: '请输入正确的库存格式'
            }).then(({ value: secStock }) => {
              const data = {
                id: row.id,
                isSecKill: 1,
                seckillPrice: parseFloat(seckillPrice),
                secStock: parseInt(secStock)
              };

              // 使用POST请求
              API.post(url + 'setseckill?isSecKill='+data.isSecKill
      ...
      setNotSecKill(row) {
        ...
        // 使用POST请求
        API.post(url + 'setnotseckill?isSecKill='+data.isSecKill, data)
            .then(res => {
              if (res.code === '200') {
